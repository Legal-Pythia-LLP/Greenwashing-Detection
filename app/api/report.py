...
def _transform(session_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Use explanation provided by quotations first
    ...
    # Then supplement key points from validations
    for v in validations if isinstance(validations, list) else []:
        ...
        if q_text or why_parts:
            evidence_groups.setdefault("External Validation Points", []).append({
                "quote": q_text,
                "why": " | ".join(why_parts)
            })
    ...
    # External information: extract simple list from news_validation/wikirate_validation (fallback empty)
    external: List[str] = []
...
async def get_report(session_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    # Build data structure compatible with previous format
    data = {
