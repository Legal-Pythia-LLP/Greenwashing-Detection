    ...
    for v in validations if isinstance(validations, list) else []:
        ...
        if q_text or why_parts:
            evidence_groups.setdefault("External validation highlights", []).append(
                {"quote": q_text, "why": " | ".join(why_parts)}
            )

    evidence = [{"type": k, "items": v} for k, v in evidence_groups.items()]

    summary_src = (
        session_data.get("final_synthesis") or session_data.get("response") or ""
    )
    summary = (
        summary_src.strip().split("\n\n")[0][:200]
        if isinstance(summary_src, str)
        else ""
    )
...
async def get_report(session_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    return {"ok": True, "data": _transform(data) | {"final_synthesis_i18n": i18n}}
